...
**Done, not yet merged to `master`** (on branch `assigned-model-display`): assigned-model display on the event detail page and all three event-listing pages (homepage, `/model/events`, `/organizer/events`) — a "TBA" placeholder until an `EventApplication` reaches `ACCEPTED`, then that model's **first name only** (not full display name — deliberately privacy-conscious for anonymous public viewers) and primary photo, via `EventApplicationService.findAssignedModel`/`findAssignedModels` and a small `AssignedModelView` record. An event has at most one assigned model — accepting a second application/invitation once one is already `ACCEPTED` for that event is now blocked with a clear error. Realizes the existing lapsed-model rule ("hidden from search, but still shown on assigned events") for the first time: display doesn't check subscription status at all, so an already-accepted model stays visible even if they later lapse.

**Done, not yet merged to `master`** (on branch `model-event-browsing`): model-facing event browsing at `/model/events` (`ModelEventBrowseController`) — defaults to the model's own profile city (via `ModelProfileService.getOrCreateProfile`), with an explicit `?city=` override (including a cleared/blank box) taking precedence over that default rather than the default reapplying itself. Linked from the model dashboard's "Browse Events" card, which previously pointed at the public homepage search.